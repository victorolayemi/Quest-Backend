...
dailyBreadRoute.post("/submit", async (c) => {
  ...
  const isCorrect =
    puzzle.solution.toLowerCase() === (solution || "").trim().toLowerCase();
...
dailyBreadRoute.get("/history", async (c) => {
  ...
  const history = await db.query.dailyBreadAttempt.findMany({
    where: and(
      eq(dailyBreadAttempt.userId, userId as string),
      eq(dailyBreadAttempt.solved, true),
    ),
  ...
  const verses = [
    {
      ...
      explanation:
        "A reflection on God’s boundless love and the gift of eternal life.",
    },
    {
      reference: "Philippians 4:13",
      text: "I can do all things through him who strengthens me.",
      explanation:
        "A reminder of the strength and empowerment we receive from Christ.",
    },
    {
    ...
    },
    {
      ...
      explanation:
        "Assurance that God works all things out for our ultimate good.",
    },
    {
      reference: "Isaiah 41:10",
      text: "Fear not, for I am with you; be not dismayed, for I am your God; I will strengthen you, I will help you, I will uphold you with my righteous right hand.",
      explanation:
        "A comforting promise of God’s presence and support in times of fear.",
    },
    {
      reference: "Psalm 23:1",
      text: "The Lord is my shepherd; I shall not want.",
      explanation:
        "A beautiful declaration of God’s provision and care as our Shepherd.",
    },
  ];
...
dailyBreadRoute.post("/verse-today/like", async (c) => {
  ...
  if (existingLike) {
    await db
      .delete(dailyVerseLike)
      .where(eq(dailyVerseLike.id, existingLike.id));

    await db
      .update(dailyVerseStat)
      .set({
