...
app.get("/:id", async (c) => {
  ...
  const [mc] = await db.select({ count: sql<number>`count(*)` }).from(communityMember).where(eq(communityMember.communityId, id));
  
  const members = await db.select().from(communityMember).where(and(eq(communityMember.communityId, id), eq(communityMember.userId, userId as string)));
  const member = members[0];
  
  let hasPendingRequest = false;
  if (!member && com.isPrivate) {
    const existingReqs = await db.select().from(communityJoinRequest).where(and(eq(communityJoinRequest.communityId, id), eq(communityJoinRequest.userId, userId as string), eq(communityJoinRequest.status, "PENDING")));
...
app.put("/:id/settings", async (c) => {
  ...
  if (isForumDisabledGlobally !== undefined) {
    await db.update(community).set({ isForumDisabledGlobally: !!isForumDisabledGlobally  }).where(eq(community.id, communityId as string));
  }
