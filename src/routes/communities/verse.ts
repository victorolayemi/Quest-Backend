...
app.get("/:id/verse-today", async (c) => {
  ...
  const userLike = await db.query.communityDailyVerseLike.findFirst({
    where: and(eq(communityDailyVerseLike.userId, userId as string), eq(communityDailyVerseLike.verseId, verse.id))
  });
...
app.post("/:id/verse-today/like", async (c) => {
  ...
  if (existingLike) {
    ...
    await db.update(communityDailyVerse).set({ likesCount: sql`${communityDailyVerse.likesCount} - 1`,  }).where(eq(communityDailyVerse.id, verse.id));
...
app.post("/:id/verse-today/share", async (c) => {
  ...
  await db.update(communityDailyVerse).set({ sharesCount: sql`${communityDailyVerse.sharesCount} + 1`,  }).where(eq(communityDailyVerse.id, verse.id));
