...
app.get("/posts/all", async (c) => {
  ...
  if (cursor) {
    const cursorPost = await db.query.post.findFirst({ where: eq(post.id, cursor) });
    if (cursorPost) {
      condition = and(condition, sql`${post.createdAt} < ${cursorPost.createdAt}`) as any;
    }
  }
...
app.get("/posts/user/created", async (c) => {
  ...
  const postsData = await db.query.post.findMany({
    where: eq(post.userId, userId as string),
    orderBy: [desc(post.createdAt)],
    with: {
      user: { columns: { firstName: true, lastName: true, username: true, avatarUrl: true } },
      community: { columns: { name: true } },
...
// GET /communities/posts/:postId — fetch a single post by ID (used for deep linking)
app.get("/posts/:postId", async (c) => {
  ...
  const [cc] = await db.select({ count: sql<number>`count(*)` }).from(comment).where(eq(comment.postId, postId));
  return c.json({ ...postRes, _count: { comments: Number(cc.count) } });
});
...
app.get("/:id/posts", async (c) => {
  ...
  let conditions = eq(post.communityId, communityId);
  
  if (cursor) {
    const cursorPost = await db.query.post.findFirst({ where: eq(post.id, cursor) });
    if (cursorPost) {
      conditions = and(conditions, sql`${post.createdAt} < ${cursorPost.createdAt}`) as any;
    }
  }
...
app.post("/posts", checkCommunityRestriction, async (c) => {
  ...
  const members = await db.select({ userId: communityMember.userId }).from(communityMember).where(and(eq(communityMember.communityId, communityId), not(eq(communityMember.userId, userId as string))));
...
app.put("/posts/:id", async (c) => {
  ...
  const [updated] = await db.update(post).set({ text, image,  }).where(eq(post.id, id)).returning();
...
app.delete("/posts/:id", async (c) => {
  ...
  const mem = await db.query.communityMember.findFirst({ where: and(eq(communityMember.communityId, p.communityId), eq(communityMember.userId, userId as string)) });
...
app.post("/posts/:id/react", async (c) => {
  ...
  const existing = await db.query.postReaction.findFirst({ where: and(eq(postReaction.postId, postId), eq(postReaction.userId, userId as string), eq(postReaction.emoji, emoji)) });
...
app.post("/posts/:postId/comments", checkCommunityRestriction, async (c) => {
  ...
  if (parentId) {
    ...
    const p = await db.query.post.findFirst({ where: eq(post.id, postId as string) });
    if (p && p.userId !== userId) {
      await dispatchNotification({
        db,
        userId: p.userId,
...
app.post("/posts/:postId/comments/:commentId/react", async (c) => {
  ...
  const existing = await db.query.commentReaction.findFirst({ where: and(eq(commentReaction.commentId, commentId), eq(commentReaction.userId, userId as string), eq(commentReaction.emoji, emoji)) });
  if (existing) {
...
app.put("/comments/:id", async (c) => {
  ...
  const [updated] = await db.update(comment).set({ text,  }).where(eq(comment.id, id)).returning();
  return c.json(updated);
});
...
app.delete("/comments/:id", async (c) => {
  ...
  const mem = await db.query.communityMember.findFirst({ where: and(eq(communityMember.communityId, com.post.id), eq(communityMember.userId, userId as string)) });
  const isAdmin = mem?.role === "ADMIN";
