...
app.get("/:id/events", async (c) => {
  ...
  let conditions = eq(communityEvent.communityId, communityId);
  if (cursor) {
    const cursorEvent = await db.query.communityEvent.findFirst({ where: eq(communityEvent.id, cursor) });
    if (cursorEvent) {
      conditions = and(conditions, sql`${communityEvent.createdAt} < ${cursorEvent.createdAt}`) as any;
    }
  }
...
app.put("/:id/events/:eventId", async (c) => {
  ...
  const [updated] = await db.update(communityEvent).set({
    title, description, date, time, location, link, imageUrl, 
...
app.post("/events/:id/attend", async (c) => {
  ...
  const existing = await db.query.eventAttendee.findFirst({ where: and(eq(eventAttendee.eventId, eventId), eq(eventAttendee.userId, userId as string)) });
  if (existing) {
