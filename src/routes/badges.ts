...
import { badge, earnedBadge, user } from "../db/schema";
...
badges.get("/progress", async (c) => {
  ...
  const allBadges = await db.query.badge.findMany();
  const earned = await db.query.earnedBadge.findMany({
    where: eq(earnedBadge.userId, userId),
  });
  const earnedBadgeIds = earned.map((e: any) => e.badgeId);

  const progress = await Promise.all(
    allBadges.map(async (badgeObj: any) => {
      let currentVal = 0;
      ...
      const percentage = Math.min(
        Math.floor((currentVal / badgeObj.criteriaValue) * 100),
        100,
      );
...
badges.post("/:id/claim", async (c) => {
  ...
  const earned = await db.query.earnedBadge.findFirst({
    where: and(
      eq(earnedBadge.userId, userId),
      eq(earnedBadge.badgeId, badgeId),
    ),
  });

  if (!earned) {
    const badgeObj = await db.query.badge.findFirst({
