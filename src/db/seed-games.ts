...
async function main() {
  ...
  await db
    ...
    .onConflictDoUpdate({
      target: schema.gameSettings.gameType,
      set: {
        totalQuestions: 10,
        durationSecs: 120,
      },
  ...
  const wmqInsert = wordMatchQuestions.map((q) => ({
    id: crypto.randomUUID(),
    ...q,
  }));
  await db.insert(schema.wordMatchQuestion).values(wmqInsert);
  ...
  const wcqInsert = wordCrossQuestions.map((q) => ({
    id: crypto.randomUUID(),
    ...q,
  }));
  await db.insert(schema.wordCrossQuestion).values(wcqInsert);
  ...
  const bqqInsert = bibleQuizQuestions.map((q) => ({
    id: crypto.randomUUID(),
    ...q,
  }));
  await db.insert(schema.bibleQuizQuestion).values(bqqInsert);
